...
export function Login() {
  ...
  const { saveAuth, setCurrentUser } = useAuth() // ✅ Get saveAuth and setCurrentUser from context
  ...
  const formik = useFormik({
    ...
    ...user,
    email: values.username, // Assuming username is an email
    password: values.password, // Store password (if needed)
    first_name: '',
    last_name: '',
  }
console.log(userWithDefaults)
  ...
  saveAuth({ api_token: token }) // Save token in context
  setCurrentUser(userWithDefaults) // ✅ Fixes the TypeScript issue
